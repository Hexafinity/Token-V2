...
contract HexaFinityToken is IHexaFinity, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "HexaFinity: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "HexaFinity: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router, address _taxReceiver) public initializer {
        ...
        __HexaFinity_init_unchained(_router, _taxReceiver);
    }

    function __HexaFinity_init_unchained(address _router, address _taxReceiver) internal initializer {
        _name = "HexaFinity";
        _symbol = "HEXA";
        _decimals = 18;
    ...
    preventBlacklisted(_account, "HexaFinity: Selected account is in blacklist")
    {
        require(_account != address(0), "HexaFinity: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "HexaFinity: Invalid address");
        require(_accountsTier[_account] > 0, "HexaFinity: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "HexaFinity: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "HexaFinity: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "HexaFinity: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "HexaFinity: Owner address is blacklisted")
    preventBlacklisted(spender, "HexaFinity: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "HexaFinity: Address is blacklisted")
    preventBlacklisted(from, "HexaFinity: From address is blacklisted")
    preventBlacklisted(to, "HexaFinity: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    external
    preventBlacklisted(account, "HexaFinity: Migrated account is blacklisted")
    override
    {
        require(migration != address(0), "HexaFinity: Migration is not started");
        require(_msgSender() == migration, "HexaFinity: Not Allowed");
